    ...
    render() {
        ...
        return <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-start' }}>
            ...
            <div style={{ width: '1200px', display: 'flex', justifyContent: 'center' }}>
                {contentDisplay === this.CONTENT[0] ? < LessonListContainer lessonsOwner={ownerUser} loggedUserId={loggedUserId} courseId={courseId} /> : <UserListContainer toggleShowChartClick={this.toggleShowChartClick} courseId={courseId} />}
            </div>
            {contentDisplay !== this.CONTENT[0] && showCourseResultChartForUser && <CourseResultContainer userId={showCourseResultChartForUser} />}
        </div >
    }
}
